...
/*
	...
	gameStat: 遊戲歷史狀態
*/
bool GetBoard(int& id_package, int mapStat[12][12], int gameStat[12][12])
{
	if (socketServer == INVALID_SOCKET)
	{
		_ConnectToServer();
	...
	if (!_RecvFromSocket(socketServer, 8, rbHeader))
	{
		...
		closesocket(socketServer);
		socketServer = INVALID_SOCKET;
		return GetBoard(id_package, mapStat, gameStat);
	}
	int codeHeader = *((int*)rbHeader);
	id_package = *((int*)(rbHeader + 4));
	if (codeHeader == 0)
	...
	for (int i = 0; i < 12; ++i)
	{
		for (int j = 0; j < 12; ++j)
		{
			if (!_RecvFromSocket(socketServer, 4, (BYTE*)&(gameStat[i][j])))
			{
				printf("[Error] : Connection lose, trying to reconnect...\n");
				closesocket(socketServer);
				socketServer = INVALID_SOCKET;
